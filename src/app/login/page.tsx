    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        .then(data => {
          ...
          } else if (data.status === 'nuevo' || data.registro === 0 || data.registro !== 1) {
            iniciarRegistro(dni);
          } else if (data.status === 'registrado' && data.registro === 1) {
      ...
      .then((stream) => {
        Swal.fire({
          ...
          if (photoResult.isConfirmed) {
            ...
            return fetch('/api/guardar_foto.php', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/x-www-form-urlencoded',
              },
          ...
          if (passwordResult?.isConfirmed) {
            ...
            return fetch('/api/registrar_usuario.php', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/x-www-form-urlencoded',
              },
  ...
  const solicitarPassword = (dni: string) => {
    Swal.fire({
      ...
      if (passwordResult.isConfirmed) {
        ...
        fetch('/api/validar_password.php', {
          method: 'POST',
